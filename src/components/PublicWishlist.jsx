...
export default function PublicWishlist() {
  ...
  const [viewer,      setViewer]      = useState(null)   // current auth user or null
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: wishlist }, followResult] = await Promise.all(queries)
      setItems(wishlist ?? [])
    ...
    <Shell>
      ...
      {activeTab === 'binder' && (
        <main className="max-w-2xl mx-auto pb-16">
          <BinderView items={items} readOnly />
        </main>
      )}
